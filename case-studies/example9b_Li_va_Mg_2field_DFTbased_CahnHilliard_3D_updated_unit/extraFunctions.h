...
std::vector<double> get_double_vector(std::string test)
{
  std::vector<std::string> vec_s=Utilities::split_string_list(test);
  std::vector<double> vec_d(vec_s.size());
  for  (unsigned int i = 0; i < vec_s.size(); ++i)
  {
    vec_d[i] =  std::stod(vec_s[i]);
  }
  return vec_d;
}
