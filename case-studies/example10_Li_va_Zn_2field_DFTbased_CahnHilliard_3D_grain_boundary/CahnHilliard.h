...
CahnHilliard<dim>::CahnHilliard()
{
  ...
  params->declare_entry("flux_mult_c","false",Patterns::Bool());
  params->declare_entry("flux_amp","1.0",Patterns::Double() );
  //
  params->declare_entry("flux_interval","false",Patterns::Bool());
  params->declare_entry("flux_interval_time","1.0",Patterns::Double());
  //
  params->declare_entry("grain_loc","40.0",Patterns::Double() );
  params->declare_entry("grain_c1_eq","0.5",Patterns::Double() );
  params->declare_entry("grain_width","0.01",Patterns::Double() );
  params->declare_entry("tau","0.02",Patterns::Double() );
                                     ...
                                     Table<1, double> &ULocalConv)
{
  ...
  //
  bool flux_mult_c = params->get_bool("flux_mult_c"); 
  double flux_time = params->get_double("flux_time");
  double flux_amp = params->get_double("flux_amp");
  //
  bool flux_interval = params->get_bool("flux_interval");
  double flux_interval_time = params->get_double("flux_interval_time");
  params->leave_subsection();
  ...
  *************************************/
  ...
  double a0 = -1.904360307391260;   //eV
  double b0 =  2.452462505353800;   //eV
  double c0 = -0.548107221698916;   //eV

  double T = 300; // K
  double kT =  8.617333262e-5 * T;  // eV

  // double a1 =  0.402413843058540;  //eV
  double a1 =  0.251186818800000;  //eV
  double r1 = 20.0;

  double d = 50;     // unit less
  double amp = 0.865; // unit less

  // ......................................................
  // grain boundary
  double PI = 3.14159265358979323846;
  params->enter_subsection("Concentration");
  double grain_loc = params->get_double("grain_loc");
  double grain_c1_eq = params->get_double("grain_c1_eq");
  double grain_width = params->get_double("grain_width");
  ...
  //
	...
	dealii::Table<1,Sacado::Fad::DFad<double> > rhs_mu1(n_q_points), rhs_mu2(n_q_points), reac_mu1(n_q_points);
  ...
  // double s = .7;
    ...
    // ......................................................
    // adding reaction boundary grain to Li-Va equation
    Point<dim> point = fe_values.quadrature_point(q); // Get the global coordinates of the quadrature point

    // double xq = (point[0] - x_min)/(x_max - x_min);
    // double yq = point[1]; 
    // double zq = (dim == 3) ? point[2] : 0.0; // Only access z if dim == 3
    // reac_mu1[q] = grain_amp/(this->current_time + 1e-4) * (c1[q]  - grain_c1_eq) * exp(-0.5 * std::pow((xq - grain_loc) / grain_width, 2));
    if ((grain_loc - grain_width/2 < point[0]) && (point[0] < (grain_loc + grain_width/2))) {
        reac_mu1[q] = 1 / tau * (c1[q] - grain_c1_eq);
    }
    else {
        reac_mu1[q] = 0.0;
    }
    ...
    f_c1 = amp * (a0 + b0 + 2.0*c0*c1[q] + (kT * d)*( -(exp(-d * c1[q])) + exp(d * (c1[q] - 1.0)) ) ); //ev

    // f_c1 += -(a1 * c2[q] * (c1[q] + 2.0*c2[q] - 1.0)) / std::pow(c1[q] - 1.0, 3); // eV 

    // f_c2 = (a1 * (c1[q] + 2*c2[q] - 1.0)) / std::pow(c1[q] - 1.0, 2);    // eV

    // f_c1 += - (a1*c2[q]*(c1[q] + 2.0*c2[q] - 1.0)*std::pow(c1[q] + 100.0*c2[q] - 1, 2))/(10000.0*std::pow(c1[q] - 1, 5));
    // f_c2 = (2*a1*(c2[q]/(c1[q] - 1.0) + 1.0/2.0)*std::pow(c2[q]/(c1[q] - 1) + 1.0/100.0, 2))/(c1[q] - 1) + (2*a1*std::pow(c2[q]/(c1[q] - 1.0) + 1.0/2.0, 2)*(c2[q]/(c1[q] - 1.0) + 1.0/100.0))/(c1[q] - 1.0);
 
    f_c1 += - (2.0*a1*c2[q]*c2[q]*r1*exp(-(c2[q]*c2[q]*r1)/std::pow(c1[q] - 1.0,2)))/std::pow(c1[q] - 1.0,3) - (2.0*a1*c2[q]*r1*exp(-r1*std::pow(c2[q]/(c1[q] - 1.0) + 1.0/2.0,2))*(c2[q]/(c1[q] - 1.0) + 1.0/2.0))/std::pow(c1[q] - 1.0,2);

    f_c2 = (2.0*a1*r1*exp(-r1*std::pow(c2[q]/(c1[q] - 1.0) + 1.0/2.0,2))*(c2[q]/(c1[q] - 1.0) + 1.0/2.0))/(c1[q] - 1.0) + (2.0*a1*c2[q]*r1*exp(-(c2[q]*c2[q]*r1)/std::pow(c1[q] - 1.0,2)))/std::pow(c1[q] - 1.0,2);

    rhs_mu1[q] = f_c1 - mu1[q];   //[eV]
    rhs_mu2[q] = f_c2 - mu2[q];   //[eV]
  ...
  // this->ResidualEq.residualForDiffusionEq(fe_values, c2_dof, R, c2, c2_conv, j_c2);
	...
	// this->ResidualEq.residualForDiffusionEq(fe_values, mu1_dof, R, c1, c1_conv, j_c1);
  this->ResidualEq.residualForDiff_ReacEq(fe_values, mu1_dof, R, c1, c1_conv, j_c1, reac_mu1);
  this->ResidualEq.residualForDiffusionEq(fe_values, mu2_dof, R, c2, c2_conv, j_c2);

	this->ResidualEq.residualForPoissonEq(fe_values, c1_dof, R, kappa_c1_grad, rhs_mu1);
	this->ResidualEq.residualForPoissonEq(fe_values, c2_dof, R, kappa_c2_grad, rhs_mu2);
  ...
  // -------------------
   if (flux_interval)
  {
    // oscillatory behavior with near-binary transitions between 0 and 1 every flux_interval_time minutes
    flux_amp *= 0.5 * (1.0 + std::tanh(5.0 * std::sin(PI / flux_interval_time * this->current_time)));
  }
  ...
  if (this->current_time > flux_time)
  {
    ...
    for (unsigned int faceID = 0; faceID < 2 * dim; faceID++)
    {
      ...
      {   
        ...
        if (std::abs(cell->face(faceID)->center()[2] - z_min) < 1.0e-8) // bottom boundary 
        {
          if (flux_mult_c)
          {
            double mm = 0.;
            double c3 = 0.;
            for (unsigned int vi = 0; vi < vertices_per_cell; ++vi)
            {
              const double z_coordinate = cell->vertex(vi)[2]; // cell->face(faceID)->vertex(vertex)[2];
              if(std::abs(z_coordinate - z_min) < 1.0e-8) // bottom boundary 
              {
                mm += 1;
                const unsigned int c1_local_dof = vi * dofs_per_vertex + c1_dof;
                const unsigned int c2_local_dof = vi * dofs_per_vertex + c2_dof;

                double c3_vi = 1 - ULocalConv[c1_local_dof] - ULocalConv[c2_local_dof]; // 1-c1-c2

                if (c3_vi < 0.0){
                  c3_vi = 0.0;
                }
                c3 += c3_vi;
                // this->pcout << c3_vi <<' ' << mm << std::endl;
              }
            }
            c3 /= mm;
            // this->pcout << c3 <<'- ' << mm << std::endl;
            if (c3 < 0.8)
              flux = flux_amp;
          }
          else
      ...
      this->ResidualEq.residualForNeummanBC(fe_values, fe_face_values, mu1_dof, R, flux); // minus flux for inbound // plus fulx for outbound
    }
  }
}


// // Assuming you have initialized fe_face_values as described earlier
// // Assuming you have a DoFHandler object named dof_handler and a Vector object named solution
...
// {
//     const double weight = fe_face_values.JxW(q_point);
//     weight_sum += weight;

//     for (unsigned int vertex = 0; vertex < GeometryInfo<dim>::vertices_per_face; ++vertex)
//     {
//         const unsigned int vertex_dof_index = cell->face(faceID)->vertex_dof_index(vertex, 0);
//         const double dof_value = solution(vertex_dof_index);
        
//         dof_sum += weight * dof_value;
//     }
// }

// const double average_dof = dof_sum / weight_sum;
  ...
  for (; cell != endc; ++cell)
  {
    if (cell->is_locally_owned()) 
    {
      ...
      for (unsigned int i = 0; i < vertices_per_cell; ++i)
      {
        ...
        // N00 = 1.0 - N10 - N01;
        this->solution(cell->vertex_dof_index(i, out1_dof , cell->active_fe_index())) = 1.0 - c1_value - c2_value; // c3 = 1-c1-c2
      }
    }
  }
